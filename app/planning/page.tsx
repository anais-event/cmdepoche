...
export default function SemainePage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const [{ data: weeklyPosts }, { data: b }] = await Promise.all([
        supabase.from('weekly_posts').select('*').eq('user_id', session.user.id).order('created_at', { ascending: false }),
        supabase.from('brand_brain').select('objective, posting_frequency').eq('user_id', session.user.id).single(),
      ]);
      setBrain(b);
      setPosts((weeklyPosts as Post[]) || []);
      setLoading(false);
    };
    load();
  }, [router]);

  const allApproved = posts.length > 0 && posts.every((p) => p.state === 'approved');
      ...
      {posts.length === 0 ? (
        ...
        <>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className={`text-[10px] font-medium px-2 py-0.5 rounded-pill ${STATE_COLORS[post.state] || ''}`}>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      ...
                      <span className="text-[10px] text-muted">{post.format}</span>
                      {post.score > 0 && (
                        <span className="text-[10px] text-terra font-medium ml-auto">Score {post.score}/100</span>
                      )}
                    </div>
                  </div>
                </button>
          ...
          <div className="mt-auto space-y-2">
            ...
            >
              <Send size={16} />
              {allApproved ? 'Programmer ma semaine' : `${posts.filter((p) => p.state === 'approved').length}/${posts.length} validés`}
            </button>
            {!allApproved && (
